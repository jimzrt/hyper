...
{
    public class COMMAND_CLASS_NETWORK_MANAGEMENT_INSTALLATION_MAINTENANCE_V2
    {
        ...
        public class RSSI_REPORT
        {
            public const byte ID = 0x08;
            public byte channel1Rssi;
            public byte channel2Rssi;
            public byte channel3Rssi;
            public static implicit operator RSSI_REPORT(byte[] data)
            {
                ...
                {
                    int index = 2;
                    ret.channel1Rssi = data.Length > index ? data[index++] : (byte)0x00;
                    ret.channel2Rssi = data.Length > index ? data[index++] : (byte)0x00;
                    ret.channel3Rssi = data.Length > index ? data[index++] : (byte)0x00;
                }
                return ret;
            }
            public static implicit operator byte[](RSSI_REPORT command)
            {
                ...
                ret.Add(COMMAND_CLASS_NETWORK_MANAGEMENT_INSTALLATION_MAINTENANCE_V2.ID);
                ret.Add(ID);
                ret.Add(command.channel1Rssi);
                ret.Add(command.channel2Rssi);
                ret.Add(command.channel3Rssi);
                return ret.ToArray();
            }
        }
    }
