...
{
    public class COMMAND_CLASS_SCHEDULE_V4
    {
        ...
        public class SCHEDULE_SUPPORTED_GET
        {
            ...
            public static implicit operator SCHEDULE_SUPPORTED_GET(byte[] data)
            {
                ...
                {
                    int index = 2;
                    ret.scheduleIdBlock = data.Length > index ? data[index++] : (byte)0x00;
                }
                return ret;
            }
            public static implicit operator byte[](SCHEDULE_SUPPORTED_GET command)
            {
                ...
                ret.Add(ID);
                ret.Add(command.scheduleIdBlock);
                return ret.ToArray();
            }
        }
        public class SCHEDULE_SUPPORTED_REPORT
        {
            ...
            public byte scheduleIdBlock;
            public byte numberOfSupportedScheduleBlocks;
            public static implicit operator SCHEDULE_SUPPORTED_REPORT(byte[] data)
            {
                ...
                if (data != null)
                {
                    ...
                    ret.scheduleIdBlock = data.Length > index ? data[index++] : (byte)0x00;
                    ret.numberOfSupportedScheduleBlocks = data.Length > index ? data[index++] : (byte)0x00;
                }
                return ret;
            }
            public static implicit operator byte[](SCHEDULE_SUPPORTED_REPORT command)
            {
                ...
                ret.Add(command.scheduleIdBlock);
                ret.Add(command.numberOfSupportedScheduleBlocks);
                return ret.ToArray();
            }
        }
        ...
        public class SCHEDULE_SUPPORTED_COMMANDS_GET
        {
        ...
        public class SCHEDULE_SUPPORTED_COMMANDS_REPORT
        {
            public const byte ID = 0x0B;
            public byte scheduleIdBlock;
            public byte commandClassListLength;
            public class TVG1
            {
                public byte commandClass;
                public byte supportedCommandListLength;
            ...
            public static implicit operator SCHEDULE_SUPPORTED_COMMANDS_REPORT(byte[] data)
            {
                ...
                if (data != null)
                {
                    int index = 2;
                    ret.scheduleIdBlock = data.Length > index ? data[index++] : (byte)0x00;
                    ret.commandClassListLength = data.Length > index ? data[index++] : (byte)0x00;
                    ret.vg1 = new List<TVG1>();
                    for (int j = 0; j < ret.commandClassListLength; j++)
                    {
                        TVG1 tmp = new TVG1();
                        tmp.commandClass = data.Length > index ? data[index++] : (byte)0x00;
                        tmp.supportedCommandListLength = data.Length > index ? data[index++] : (byte)0x00;
                        tmp.supportedCommand = new List<byte>();
                        for (int i = 0; i < tmp.supportedCommandListLength; i++)
                        {
                            tmp.supportedCommand.Add(data.Length > index ? data[index++] : (byte)0x00);
                        }
                        ret.vg1.Add(tmp);
                    }
                }
            ...
            public static implicit operator byte[](SCHEDULE_SUPPORTED_COMMANDS_REPORT command)
            {
                ...
                ret.Add(ID);
                ret.Add(command.scheduleIdBlock);
                ret.Add(command.commandClassListLength);
                if (command.vg1 != null)
                {
                    foreach (var item in command.vg1)
                    {
                        ret.Add(item.commandClass);
                        ret.Add(item.supportedCommandListLength);
